...
async def create_user(user: UserCreate, current_user: dict = Depends(get_current_user)):
    ...
    if existing_user:
        raise HTTPException(
            status_code=400, detail="User already exists for this Firebase identity"
        )
...
async def update_user(
    user_id: uuid.UUID, user: UserUpdate, current_user: dict = Depends(get_current_user)
):
    user_obj = await User.get_or_none(id=user_id)
    if not user_obj:
        raise HTTPException(status_code=404, detail="User not found")
    ...
    if user_obj.firebase_uid != current_user.get("uid"):
        raise HTTPException(
            status_code=403, detail="Not authorized to update this record"
        )
...
async def delete_user(
    user_id: uuid.UUID, current_user: dict = Depends(get_current_user)
):
    user_obj = await User.get_or_none(id=user_id)
    if not user_obj:
        raise HTTPException(status_code=404, detail="User not found")
    ...
    if user_obj.firebase_uid != current_user.get("uid"):
        raise HTTPException(
            status_code=403, detail="Not authorized to delete this record"
        )
